 ...
 * @returns {H5P.MusicNotationQuestionSet} Instance
 */
H5P.MusicNotationQuestionSet = function (options, contentId, contentData) {
  if (!(this instanceof H5P.MusicNotationQuestionSet)) {
    return new H5P.MusicNotationQuestionSet(options, contentId, contentData);
  }
  ...
  var createQuestionInstancesFromQuestions = function (questions) {
    ...
    for (var i = 0; i < questions.length; i++) {
      ...
      // help multichoice instance to identify question container to which all other elements will be attached
      question.params.question_instance_num = i
      ...
      questionInstance.on('resize', function () {
        up = true;
        self.trigger('resize');
        if(this.constructor.name.toLowerCase().includes("multichoice")){
          var instanceContainer = document.querySelectorAll(`.question-container.h5p-multichoice`)
          instanceContainer?.forEach(ic => {
            ic.querySelectorAll(".notation").forEach(n => {
              this.adjustFrame(n)
            })
          })
        }
  ...
  /**
   * Targets with vibe-containers have to be observed if they are currently shown.
   * Reload vibe, if the corresponding question-container is the current slide.
   */
  function observeCurrentSlide(){
    var observer = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        var isQC = mutation.target.classList.contains("question-container")
        var isAttr = mutation.type === "attributes"
        if(isQC && isAttr){
          var changedStyle = mutation.attributeName === "style"
          var styleDeleted = mutation.target.style.display === ""
          var vibeContainer = mutation.target.querySelector(".vibe-container")
          if(changedStyle && styleDeleted && vibeContainer !== null){
            // eventlistener is in Verovioscoreeditor.WindowHandler
    ...
    observer.observe(self.$questionsContainer[0], {
      childList: true,
      subtree: true,
      attributes: true
    })
...
H5P.MusicNotationQuestionSet.prototype = Object.create(H5P.EventDispatcher.prototype);
H5P.MusicNotationQuestionSet.prototype.constructor = H5P.MusicNotationQuestionSet;
